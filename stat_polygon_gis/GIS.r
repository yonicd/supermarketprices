...
while(count<5&tol!=0){
  for(i in which(is.na(x1$lat))){
    a=data.frame(t(getGeoCode(x1$full.address[i])))
    x1$lat[i]=a$lat
    x1$lon[i]=a$lon
  }
...
projxy=project(as.matrix(x1[,c(3,2)]),proj= projstr)
...
x1=left_join(x1,isr_poly_df%>%select(OBJECTID,CITY),by=c("OBJECTID"))
