...
#Read position data for Rami Levi Stores
ramilevi=read.csv("C:/Users/yoni/Documents/GitHub/supermarketprices/Stores/ramilevistoressite.csv")

#Enter manually some stores of mega and beitan wines (modiin area)
...
pal <- colorFactor(c("navy", "red"), domain = c("Rami Levi","Mega","Beitan"))
...
m=leaflet(df)  %>% addTiles(MBTemplate) #base map of world
m
...
m=m%>%addCircleMarkers(lat=coordinates(df)[,1], #latitude
...
#add layer of statistic area polygon (blue lines)
m=m%>%addPolygons(data=bound_latlng,
